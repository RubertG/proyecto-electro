...
from ctrlVectorFuerza import *

def graficarVectores(posiciones, principal):
    prinX = principal[0][0]
    prinY = principal[0][1]
    colores = ['b', 'y', 'm', 'k', 'g']
    i = 0
    ...
    fuerza = ctrlVectorFuerza(principal, posiciones)
    ...
    plt.xlim(minX - (10**len(str(maxY)) - 5), maxX + (10**len(str(maxY)) - 4))
    plt.ylim(minY - (10**len(str(maxY)) - 5), maxY + (10**len(str(maxY)) - 4))
    plt.grid(b=True, which='major')
    plt.title('Grafica fuerza resultante')
    plt.show()


principal = [[6, -2], 20*(10**-3)]
posiciones = [[[-12, -9], 90*(10**-3)], [[-7, 6], -60*(10**-3)]]
graficarVectores(posiciones, principal)
